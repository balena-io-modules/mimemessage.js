...
Entity.prototype.toString = function (options) {
	...
	if (this._status) {
		raw += 'HTTP/1.1 ' + this._status + ' ' + STATUS_CODES[this._status] + '\r\n'
	}
		...
		for (name in this._headers) {
			...
			for (prop in this) {
				if (this.hasOwnProperty(prop)) {
					if (prop[0] !== '_') raw += prop + ': ' + this[prop] + '\r\n';
				}
			}
		}
...
function setBody(body) {
	...
	if (Array.isArray(body)) {
		boundary = randomString({length: 16})
		if (!contentType || contentType.type !== 'multipart') {
			this.contentType('multipart/mixed;boundary=' + boundary);
		} else if (!contentType.params.boundary) {
			this.contentType(contentType.fulltype + ';boundary=' + boundary);
		}
	// Single body.
	} else {
		if (!contentType || contentType.type === 'multipart') {
